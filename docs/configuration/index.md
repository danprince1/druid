...
|Property|Description|Default|
|--------|-----------|-------|
|`druid.coordinator.dutiesRunnableExecutor.threadPoolSize`| The number of threads used by the `ScheduledExecutorService` that executes `DutiesRunnable` objects.  This must be greater than 1 if `loadPrimaryReplicantSeparately` is set.|1|
...
|`druid.coordinator.loadPrimaryReplicantSeparately`| Flag that indicates if the Coordinator should put primary replicant loading for used segments on its own event loop. This will require more memory and CPU to be used by the coordinator, but will ensure that the loading of primary replicas for used segments is not blocked by other coordinator jobs. The default of false is likely fine for most clusters. Enabling it could make sense if you regularly have to wait longer than you'd like for coordination cycles to complete before primary replicants are loaded for used segments that are not being served by the cluster.  If true, `druid.coordinator.dutiesRunnableExecutor.threadPoolSize` must be set to something greater than 1.|false|
|`druid.coordinator.period.primaryReplicantLoaderPeriod`| Only used if `druid.coordinator.loadPrimaryReplicantSeparately=true`. How often the coordinator will execute its job to load primary replicants for used segments. This is the delay after completion of the previous cycle. The N + 1th cycle will start the specified period of time after the Nth cycle completes.|PT60S|
...
| `druid.manager.segments.pollingType`  | The type of segment polling done. `full` means the entire contents of the druid_segments table is read on every poll; `incremental` (experimental) means only new and changed segments will be selected.  NOTE: if segment killing is enabled (via `druid.coordinator.kill.on`), `incremental` mode requires `druid.coordinator.kill.bufferPeriod` be set to a period larger than `druid.manager.segments.pollDuration`. | full     |
