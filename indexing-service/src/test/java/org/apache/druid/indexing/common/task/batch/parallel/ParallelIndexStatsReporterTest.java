...
public class ParallelIndexStatsReporterTest
{
  ...
  @Test
  public void testGetRowStatsAndUnparseableEventsForRunningTasksHappy() throws IOException
  {
    InputStream inputStream = getClass().getClassLoader().getResourceAsStream("ingestionStatsAndErrors.json");
    Assert.assertNotNull("failed to find test file", inputStream);
    @SuppressWarnings("Convert2Diamond")
    Map<String, Object> report = OBJECT_MAPPER.readValue(inputStream, new TypeReference<Map<String, Object>>()
    {
    });
    Mockito.when(supervisorTask.fetchTaskReport("some_task_id")).thenReturn(report);
    ...
    RowIngestionMetersTotals totals = reporter.getRowStatsAndUnparseableEventsForRunningTasks(
        supervisorTask,
        ImmutableSet.of("some_task_id"),
        unparseableEvents,
        true
    );

    Assert.assertEquals(ROW_INGESTION_METERS_TOTALS, totals);
    Assert.assertEquals(1, unparseableEvents.size());
    ParseExceptionReport parseExceptionReport = OBJECT_MAPPER.convertValue(
        unparseableEvents.get(0),
        ParseExceptionReport.class
    );
    Assert.assertEquals(PARSE_EXCEPTION_REPORT, parseExceptionReport);
  }
